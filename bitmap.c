...
Bitmap *read_header() {
    ...
    //Read data before header size into temp, get header size and allocate space
    //for header
    if(fread(temp, BMP_HEADER_SIZE_OFFSET, 1, stdin) != 1) {
        fprintf(stderr, "ERROR: fread failed\n");
        exit(1);
    }
    if(fread(&bitmap->headerSize, 4, 1, stdin) != 1) {
        fprintf(stderr, "ERROR: fread failed\n");
        exit(1);
    }
    ...
    //Concatenate 3 parts of the header
    memcpy(bitmap->header, temp, BMP_HEADER_SIZE_OFFSET);
    memcpy(bitmap->header + BMP_HEADER_SIZE_OFFSET, &bitmap->headerSize, 4);
    if(fread(bitmap->header + BMP_HEADER_SIZE_OFFSET + 4,
        bitmap->headerSize - BMP_HEADER_SIZE_OFFSET - 4, 1, stdin) != 1) {
        fprintf(stderr, "ERROR: fread failed\n");
        exit(1);
    }

    //Update width and height according to the header 
    memcpy(&bitmap->width, bitmap->header + BMP_WIDTH_OFFSET, 4);
    memcpy(&bitmap->height, bitmap->header + BMP_HEIGHT_OFFSET, 4);
...
void scale(Bitmap *bmp, int scale_factor) {
    ...
    //Update width, height and scale factor
    bmp->width = bmp->width * scale_factor;
    bmp->height = bmp->height * scale_factor;
    file_size = bmp->width * bmp->height * 3 + bmp->headerSize;
    bmp->scale_factor = scale_factor;

    //Update header according to the values above
    memcpy(bmp->header + BMP_FILE_SIZE_OFFSET, &file_size, 4);
    memcpy(bmp->header + BMP_WIDTH_OFFSET, &bmp->width, 4);
    memcpy(bmp->header + BMP_HEIGHT_OFFSET, &bmp->height, 4);
}
