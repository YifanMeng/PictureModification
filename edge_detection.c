...
void edge_detection_filter(Bitmap *bmp) {
    ...
    for (row = 0; row < height; row++){
        for (col = 0; col < width; col++) {
            //left most pixel
            if (col == 0) {
                new_pixel = apply_edge_detection_kernel(rows[0], rows[1], 
                    rows[2]);
            //right most pixel
            } else if (col == (width - 1)) {
                new_pixel = apply_edge_detection_kernel(rows[0] + col - 2, 
                    rows[1] + col - 2, rows[2] + col - 2);
            //pixels in the middle
            } else {
                new_pixel = apply_edge_detection_kernel(rows[0] + col - 1, 
        ...
        //Read in a new row after processing every row, except for the first and
        //the last row
